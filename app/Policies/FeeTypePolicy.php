<?php

namespace App\Policies;

use App\Models\User;
use App\Models\Backend\Account\FeeType;
use Illuminate\Auth\Access\HandlesAuthorization;

class FeeTypePolicy
{
    use HandlesAuthorization;

    /**
     * Determine whether the feeType can view any models.
     *
     * @param  App\Models\User  $user
     * @return mixed
     */
    public function viewAny(User $user)
    {
        return true;
    }

    /**
     * Determine whether the feeType can view the model.
     *
     * @param  App\Models\User  $user
     * @param  App\Models\FeeType  $model
     * @return mixed
     */
    public function view(User $user, FeeType $model)
    {
        return true;
    }

    /**
     * Determine whether the feeType can create models.
     *
     * @param  App\Models\User  $user
     * @return mixed
     */
    public function create(User $user)
    {
        return true;
    }

    /**
     * Determine whether the feeType can update the model.
     *
     * @param  App\Models\User  $user
     * @param  App\Models\FeeType  $model
     * @return mixed
     */
    public function update(User $user, FeeType $model)
    {
        return true;
    }

    /**
     * Determine whether the feeType can delete the model.
     *
     * @param  App\Models\User  $user
     * @param  App\Models\FeeType  $model
     * @return mixed
     */
    public function delete(User $user, FeeType $model)
    {
        return true;
    }

    /**
     * Determine whether the user can delete multiple instances of the model.
     *
     * @param  App\Models\User  $user
     * @param  App\Models\FeeType  $model
     * @return mixed
     */
    public function deleteAny(User $user)
    {
        return true;
    }

    /**
     * Determine whether the feeType can restore the model.
     *
     * @param  App\Models\User  $user
     * @param  App\Models\FeeType  $model
     * @return mixed
     */
    public function restore(User $user, FeeType $model)
    {
        return false;
    }

    /**
     * Determine whether the feeType can permanently delete the model.
     *
     * @param  App\Models\User  $user
     * @param  App\Models\FeeType  $model
     * @return mixed
     */
    public function forceDelete(User $user, FeeType $model)
    {
        return false;
    }
}
